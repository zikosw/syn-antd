...
var minPieces=[],shortPieces=[],longPieces=[],mixedPieces=[],i;for(i=0;7>i;i++){var mom=createUTC([2E3,1]).day(i);var minp=this.weekdaysMin(mom,"");var shortp=this.weekdaysShort(mom,"");mom=this.weekdays(mom,"");minPieces.push(minp);shortPieces.push(shortp);longPieces.push(mom);mixedPieces.push(minp);mixedPieces.push(shortp);mixedPieces.push(mom)}minPieces.sort(cmpLenRev);shortPieces.sort(cmpLenRev);longPieces.sort(cmpLenRev);mixedPieces.sort(cmpLenRev);for(i=0;7>i;i++)shortPieces[i]=regexEscape(shortPieces[i]),
longPieces[i]=regexEscape(longPieces[i]),mixedPieces[i]=regexEscape(mixedPieces[i]);this._weekdaysMinRegex=this._weekdaysShortRegex=this._weekdaysRegex=new RegExp("^("+mixedPieces.join("|")+")","i");this._weekdaysStrictRegex=new RegExp("^("+longPieces.join("|")+")","i");this._weekdaysShortStrictRegex=new RegExp("^("+shortPieces.join("|")+")","i");this._weekdaysMinStrictRegex=new RegExp("^("+minPieces.join("|")+")","i")}function hFormat(){return this.hours()%12||12}function meridiem(token,lowercase){addFormatToken(token,
0,0,function(){return this.localeData().meridiem(this.hours(),this.minutes(),lowercase)})}function matchMeridiem(isStrict,locale){return locale._meridiemParse}function normalizeLocale(key){return key?key.toLowerCase().replace("_","-"):key}function loadLocale(name){var oldLocale=null;if(!locales[name]&&"undefined"!==typeof module&&module&&module.exports)try{oldLocale=globalLocale._abbr,require("./locale/"+name),getSetGlobalLocale(oldLocale)}catch(e){}return locales[name]}function getSetGlobalLocale(key,
values){key&&((values=isUndefined(values)?getLocale(key):defineLocale(key,values))?globalLocale=values:"undefined"!==typeof console&&console.warn&&console.warn("Locale "+key+" not found. Did you forget to load it?"));return globalLocale._abbr}function defineLocale(name,config){if(null!==config){var locale=baseConfig;config.abbr=name;if(null!=locales[name])deprecateSimple("defineLocaleOverride","use moment.updateLocale(localeName, config) to change an existing locale. moment.defineLocale(localeName, config) should only be used for creating a new locale See http://momentjs.com/guides/#/warnings/define-locale/ for more info."),
...
loadLocale(split.slice(0,j).join("-"))){key=locale$jscomp$0;break a}if(next&&next.length>=j&&compareArrays(split,next,!0)>=j-1)break;j--}locale++}key=globalLocale}return key}function checkOverflow(m){var overflow;(overflow=m._a)&&-2===getParsingFlags(m).overflow&&(overflow=0>overflow[MONTH]||11<overflow[MONTH]?MONTH:1>overflow[DATE]||overflow[DATE]>daysInMonth(overflow[YEAR],overflow[MONTH])?DATE:0>overflow[HOUR]||24<overflow[HOUR]||24===overflow[HOUR]&&(0!==overflow[MINUTE]||0!==overflow[SECOND]||
0!==overflow[MILLISECOND])?HOUR:0>overflow[MINUTE]||59<overflow[MINUTE]?MINUTE:0>overflow[SECOND]||59<overflow[SECOND]?SECOND:0>overflow[MILLISECOND]||999<overflow[MILLISECOND]?MILLISECOND:-1,getParsingFlags(m)._overflowDayOfYear&&(overflow<YEAR||overflow>DATE)&&(overflow=DATE),getParsingFlags(m)._overflowWeeks&&-1===overflow&&(overflow=WEEK),getParsingFlags(m)._overflowWeekday&&-1===overflow&&(overflow=WEEKDAY),getParsingFlags(m).overflow=overflow);return m}function defaults(a,b,c){return null!=
a?a:null!=b?b:c}function configFromArray(config){var input=[];if(!config._d){var currentDate=new Date(hooks.now());currentDate=config._useUTC?[currentDate.getUTCFullYear(),currentDate.getUTCMonth(),currentDate.getUTCDate()]:[currentDate.getFullYear(),currentDate.getMonth(),currentDate.getDate()];if(config._w&&null==config._a[DATE]&&null==config._a[MONTH]){var w=config._w;if(null!=w.GG||null!=w.W||null!=w.E){var dow=1;var doy=4;var weekYear=defaults(w.GG,config._a[YEAR],weekOfYear(createLocal(),1,
4).year);var week=defaults(w.W,1);var weekday=defaults(w.E,1);if(1>weekday||7<weekday)var i=!0}else if(dow=config._locale._week.dow,doy=config._locale._week.doy,week=weekOfYear(createLocal(),dow,doy),weekYear=defaults(w.gg,config._a[YEAR],week.year),week=defaults(w.w,week.week),null!=w.d){if(weekday=w.d,0>weekday||6<weekday)i=!0}else if(null!=w.e){if(weekday=w.e+dow,0>w.e||6<w.e)i=!0}else weekday=dow;1>week||week>weeksInYear(weekYear,dow,doy)?getParsingFlags(config)._overflowWeeks=!0:null!=i?getParsingFlags(config)._overflowWeekday=
!0:(i=dayOfYearFromWeeks(weekYear,week,weekday,dow,doy),config._a[YEAR]=i.year,config._dayOfYear=i.dayOfYear)}if(null!=config._dayOfYear){i=defaults(config._a[YEAR],currentDate[YEAR]);if(config._dayOfYear>(isLeapYear(i)?366:365)||0===config._dayOfYear)getParsingFlags(config)._overflowDayOfYear=!0;i=createUTCDate(i,0,config._dayOfYear);config._a[MONTH]=i.getUTCMonth();config._a[DATE]=i.getUTCDate()}for(i=0;3>i&&null==config._a[i];++i)config._a[i]=input[i]=currentDate[i];for(;7>i;i++)config._a[i]=input[i]=
null==config._a[i]?2===i?1:0:config._a[i];24===config._a[HOUR]&&0===config._a[MINUTE]&&0===config._a[SECOND]&&0===config._a[MILLISECOND]&&(config._nextDay=!0,config._a[HOUR]=0);config._d=(config._useUTC?createUTCDate:createDate).apply(null,input);input=config._useUTC?config._d.getUTCDay():config._d.getDay();null!=config._tzm&&config._d.setUTCMinutes(config._d.getUTCMinutes()-config._tzm);config._nextDay&&(config._a[HOUR]=24);config._w&&"undefined"!==typeof config._w.d&&config._w.d!==input&&(getParsingFlags(config).weekdayMismatch=
...
duration.months=-duration.months),key=duration):key={milliseconds:0,months:0},duration={},duration.ms=key.milliseconds,duration.M=key.months);duration=new Duration(duration);isDuration(input)&&hasOwnProp(input,"_locale")&&(duration._locale=input._locale);return duration}function parseIso(inp,sign){inp=inp&&parseFloat(inp.replace(",","."));return(isNaN(inp)?0:inp)*sign}function positiveMomentsDifference(base,other){var res={};res.months=other.month()-base.month()+12*(other.year()-base.year());base.clone().add(res.months,
"M").isAfter(other)&&--res.months;res.milliseconds=+other-+base.clone().add(res.months,"M");return res}function createAdder(direction,name){return function(val,period){if(null!==period&&!isNaN(+period)){deprecateSimple(name,"moment()."+name+"(period, number) is deprecated. Please use moment()."+name+"(number, period). See http://momentjs.com/guides/#/warnings/add-inverted-param/ for more info.");var tmp=val;val=period;period=tmp}val=createDuration("string"===typeof val?+val:val,period);addSubtract(this,
val,direction);return this}}function addSubtract(mom,duration,isAdding,updateOffset){var milliseconds=duration._milliseconds,days=absRound(duration._days);duration=absRound(duration._months);mom.isValid()&&(updateOffset=null==updateOffset?!0:updateOffset,duration&&setMonth(mom,get(mom,"Month")+duration*isAdding),days&&set$1(mom,"Date",get(mom,"Date")+days*isAdding),milliseconds&&mom._d.setTime(mom._d.valueOf()+milliseconds*isAdding),updateOffset&&hooks.updateOffset(mom,days||duration))}function monthDiff(a,
b){var wholeMonthDiff=12*(b.year()-a.year())+(b.month()-a.month()),anchor=a.clone().add(wholeMonthDiff,"months");0>b-anchor?(a=a.clone().add(wholeMonthDiff-1,"months"),b=(b-anchor)/(anchor-a)):(a=a.clone().add(wholeMonthDiff+1,"months"),b=(b-anchor)/(a-anchor));return-(wholeMonthDiff+b)||0}function locale$jscomp$1(key){if(void 0===key)return this._locale._abbr;key=getLocale(key);null!=key&&(this._locale=key);return this}function localeData(){return this._locale}function mod$1(dividend,divisor){return(dividend%
divisor+divisor)%divisor}function localStartOfDate(y,m,d){return 100>y&&0<=y?new Date(y+400,m,d)-126227808E5:(new Date(y,m,d)).valueOf()}function utcStartOfDate(y,m,d){return 100>y&&0<=y?Date.UTC(y+400,m,d)-126227808E5:Date.UTC(y,m,d)}function addWeekYearFormatToken(token,getter){addFormatToken(0,[token,token.length],0,getter)}function getSetWeekYearHelper(input,week,weekday,dow,doy){if(null==input)return weekOfYear(this,dow,doy).year;var weeksTarget=weeksInYear(input,dow,doy);week>weeksTarget&&(week=
weeksTarget);input=dayOfYearFromWeeks(input,week,weekday,dow,doy);input=createUTCDate(input.year,0,input.dayOfYear);this.year(input.getUTCFullYear());this.month(input.getUTCMonth());this.date(input.getUTCDate());return this}function parseMs(input,array){array[MILLISECOND]=toInt(1E3*("0."+input))}function preParsePostFormat(string){return string}function get$1(format,index,field,setter){var locale=getLocale();index=createUTC().set(setter,index);return locale[field](index,format)}function listMonthsImpl(format,
index,field){isNumber(format)&&(index=format,format=void 0);format=format||"";if(null!=index)return get$1(format,index,field,"month");var out=[];for(index=0;12>index;index++)out[index]=get$1(format,index,field,"month");return out}function listWeekdaysImpl(localeSorted,format,index,field){"boolean"!==typeof localeSorted&&(index=format=localeSorted,localeSorted=!1);isNumber(format)&&(index=format,format=void 0);format=format||"";var locale=getLocale();localeSorted=localeSorted?locale._week.dow:0;if(null!=
index)return get$1(format,(index+localeSorted)%7,field,"day");locale=[];for(index=0;7>index;index++)locale[index]=get$1(format,(index+localeSorted)%7,field,"day");return locale}function addSubtract$1(duration,input,value,direction){input=createDuration(input,value);duration._milliseconds+=direction*input._milliseconds;duration._days+=direction*input._days;duration._months+=direction*input._months;return duration._bubble()}function absCeil(number){return 0>number?Math.floor(number):Math.ceil(number)}
function makeAs(alias){return function(){return this.as(alias)}}function makeGetter(name){return function(){return this.isValid()?this._data[name]:NaN}}function substituteTimeAgo(string,number,withoutSuffix,isFuture,locale){return locale.relativeTime(number||1,!!withoutSuffix,string,isFuture)}function sign(x){return(0<x)-(0>x)||+x}function toISOString$1(){if(!this.isValid())return this.localeData().invalidDate();var seconds=abs$1(this._milliseconds)/1E3,days=abs$1(this._days),months=abs$1(this._months);
var minutes=absFloor(seconds/60);var hours=absFloor(minutes/60);seconds%=60;minutes%=60;var Y=absFloor(months/12);months%=12;seconds=seconds?seconds.toFixed(3).replace(/\.?0+$/,""):"";var total=this.asSeconds();if(!total)return"P0D";var totalSign=0>total?"-":"",ymSign=sign(this._months)!==sign(total)?"-":"",daysSign=sign(this._days)!==sign(total)?"-":"";total=sign(this._milliseconds)!==sign(total)?"-":"";return totalSign+"P"+(Y?ymSign+Y+"Y":"")+(months?ymSign+months+"M":"")+(days?daysSign+days+"D":
"")+(hours||minutes||seconds?"T":"")+(hours?total+hours+"H":"")+(minutes?total+minutes+"M":"")+(seconds?total+seconds+"S":"")}var some=Array.prototype.some?Array.prototype.some:function(fun){for(var t=Object(this),len=t.length>>>0,i=0;i<len;i++)if(i in t&&fun.call(this,t[i],i,t))return!0;return!1};var momentProperties=hooks.momentProperties=[],updateInProgress=!1,deprecations={};hooks.suppressDeprecationWarnings=!1;hooks.deprecationHandler=null;var keys=Object.keys?Object.keys:function(obj){var i,
res=[];for(i in obj)hasOwnProp(obj,i)&&res.push(i);return res};var aliases={},priorities={},formattingTokens=/(\[[^\[]*\])|(\\)?([Hh]mm(ss)?|Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|Qo?|YYYYYY|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|kk?|mm?|ss?|S{1,9}|x|X|zz?|ZZ?|.)/g,localFormattingTokens=/(\[[^\[]*\])|(\\)?(LTS|LT|LL?L?L?|l{1,4})/g,formatFunctions={},formatTokenFunctions={},match1=/\d/,match2=/\d\d/,match3=/\d{3}/,match4=/\d{4}/,match6=/[+-]?\d{6}/,match1to2=/\d\d?/,match3to4=
/\d\d\d\d?/,match5to6=/\d\d\d\d\d\d?/,match1to3=/\d{1,3}/,match1to4=/\d{1,4}/,match1to6=/[+-]?\d{1,6}/,matchUnsigned=/\d+/,matchSigned=/[+-]?\d+/,matchOffset=/Z|[+-]\d\d:?\d\d/gi,matchShortOffset=/Z|[+-]\d\d(?::?\d\d)?/gi,matchWord=/[0-9]{0,256}['a-z\u00A0-\u05FF\u0700-\uD7FF\uF900-\uFDCF\uFDF0-\uFF07\uFF10-\uFFEF]{1,256}|[\u0600-\u06FF\/]{1,256}(\s*?[\u0600-\u06FF]{1,256}){1,2}/i,regexes={},tokens={},YEAR=0,MONTH=1,DATE=2,HOUR=3,MINUTE=4,SECOND=5,MILLISECOND=6,WEEK=7,WEEKDAY=8;addFormatToken("Y",
0,0,function(){var y=this.year();return 9999>=y?""+y:"+"+y});addFormatToken(0,["YY",2],0,function(){return this.year()%100});addFormatToken(0,["YYYY",4],0,"year");addFormatToken(0,["YYYYY",5],0,"year");addFormatToken(0,["YYYYYY",6,!0],0,"year");addUnitAlias("year","y");priorities.year=1;addRegexToken("Y",matchSigned);addRegexToken("YY",match1to2,match2);addRegexToken("YYYY",match1to4,match4);addRegexToken("YYYYY",match1to6,match6);addRegexToken("YYYYYY",match1to6,match6);addParseToken(["YYYYY","YYYYYY"],
YEAR);addParseToken("YYYY",function(input,array){array[YEAR]=2===input.length?hooks.parseTwoDigitYear(input):toInt(input)});addParseToken("YY",function(input,array){array[YEAR]=hooks.parseTwoDigitYear(input)});addParseToken("Y",function(input,array){array[YEAR]=parseInt(input,10)});hooks.parseTwoDigitYear=function(input){return toInt(input)+(68<toInt(input)?1900:2E3)};var getSetYear=makeGetSet("FullYear",!0);var indexOf=Array.prototype.indexOf?Array.prototype.indexOf:function(o){var i;for(i=0;i<this.length;++i)if(this[i]===
o)return i;return-1};addFormatToken("M",["MM",2],"Mo",function(){return this.month()+1});addFormatToken("MMM",0,0,function(format){return this.localeData().monthsShort(this,format)});addFormatToken("MMMM",0,0,function(format){return this.localeData().months(this,format)});addUnitAlias("month","M");priorities.month=8;addRegexToken("M",match1to2);addRegexToken("MM",match1to2,match2);addRegexToken("MMM",function(isStrict,locale){return locale.monthsShortRegex(isStrict)});addRegexToken("MMMM",function(isStrict,
locale){return locale.monthsRegex(isStrict)});addParseToken(["M","MM"],function(input,array){array[MONTH]=toInt(input)-1});addParseToken(["MMM","MMMM"],function(input,array,config,token){token=config._locale.monthsParse(input,token,config._strict);null!=token?array[MONTH]=token:getParsingFlags(config).invalidMonth=input});var MONTHS_IN_FORMAT=/D[oD]?(\[[^\[\]]*\]|\s)+MMMM?/,defaultLocaleMonthsShort="Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec".split(" ");addFormatToken("w",["ww",2],"wo","week");
addFormatToken("W",["WW",2],"Wo","isoWeek");addUnitAlias("week","w");addUnitAlias("isoWeek","W");priorities.week=5;priorities.isoWeek=5;addRegexToken("w",match1to2);addRegexToken("ww",match1to2,match2);addRegexToken("W",match1to2);addRegexToken("WW",match1to2,match2);addWeekParseToken(["w","ww","W","WW"],function(input,week,config,token){week[token.substr(0,1)]=toInt(input)});addFormatToken("d",0,"do","day");addFormatToken("dd",0,0,function(format){return this.localeData().weekdaysMin(this,format)});
addFormatToken("ddd",0,0,function(format){return this.localeData().weekdaysShort(this,format)});addFormatToken("dddd",0,0,function(format){return this.localeData().weekdays(this,format)});addFormatToken("e",0,0,"weekday");addFormatToken("E",0,0,"isoWeekday");addUnitAlias("day","d");addUnitAlias("weekday","e");addUnitAlias("isoWeekday","E");priorities.day=11;priorities.weekday=11;priorities.isoWeekday=11;addRegexToken("d",match1to2);addRegexToken("e",match1to2);addRegexToken("E",match1to2);addRegexToken("dd",
function(isStrict,locale){return locale.weekdaysMinRegex(isStrict)});addRegexToken("ddd",function(isStrict,locale){return locale.weekdaysShortRegex(isStrict)});addRegexToken("dddd",function(isStrict,locale){return locale.weekdaysRegex(isStrict)});addWeekParseToken(["dd","ddd","dddd"],function(input,week,config,token){token=config._locale.weekdaysParse(input,token,config._strict);null!=token?week.d=token:getParsingFlags(config).invalidWeekday=input});addWeekParseToken(["d","e","E"],function(input,
week,config,token){week[token]=toInt(input)});var defaultLocaleWeekdaysShort="Sun Mon Tue Wed Thu Fri Sat".split(" ");addFormatToken("H",["HH",2],0,"hour");addFormatToken("h",["hh",2],0,hFormat);addFormatToken("k",["kk",2],0,function(){return this.hours()||24});addFormatToken("hmm",0,0,function(){return""+hFormat.apply(this)+zeroFill(this.minutes(),2)});addFormatToken("hmmss",0,0,function(){return""+hFormat.apply(this)+zeroFill(this.minutes(),2)+zeroFill(this.seconds(),2)});addFormatToken("Hmm",0,
0,function(){return""+this.hours()+zeroFill(this.minutes(),2)});addFormatToken("Hmmss",0,0,function(){return""+this.hours()+zeroFill(this.minutes(),2)+zeroFill(this.seconds(),2)});meridiem("a",!0);meridiem("A",!1);addUnitAlias("hour","h");priorities.hour=13;addRegexToken("a",matchMeridiem);addRegexToken("A",matchMeridiem);addRegexToken("H",match1to2);addRegexToken("h",match1to2);addRegexToken("k",match1to2);addRegexToken("HH",match1to2,match2);addRegexToken("hh",match1to2,match2);addRegexToken("kk",
...
!0});addParseToken("hmm",function(input,array,config){var pos=input.length-2;array[HOUR]=toInt(input.substr(0,pos));array[MINUTE]=toInt(input.substr(pos));getParsingFlags(config).bigHour=!0});addParseToken("hmmss",function(input,array,config){var pos1=input.length-4,pos2=input.length-2;array[HOUR]=toInt(input.substr(0,pos1));array[MINUTE]=toInt(input.substr(pos1,2));array[SECOND]=toInt(input.substr(pos2));getParsingFlags(config).bigHour=!0});addParseToken("Hmm",function(input,array,config){config=
input.length-2;array[HOUR]=toInt(input.substr(0,config));array[MINUTE]=toInt(input.substr(config))});addParseToken("Hmmss",function(input,array,config){config=input.length-4;var pos2=input.length-2;array[HOUR]=toInt(input.substr(0,config));array[MINUTE]=toInt(input.substr(config,2));array[SECOND]=toInt(input.substr(pos2))});var getSetHour=makeGetSet("Hours",!0),baseConfig={calendar:{sameDay:"[Today at] LT",nextDay:"[Tomorrow at] LT",nextWeek:"dddd [at] LT",lastDay:"[Yesterday at] LT",lastWeek:"[Last] dddd [at] LT",
...
tzRegex=/Z|[+-]\d\d(?::?\d\d)?/,isoDates=[["YYYYYY-MM-DD",/[+-]\d{6}-\d\d-\d\d/],["YYYY-MM-DD",/\d{4}-\d\d-\d\d/],["GGGG-[W]WW-E",/\d{4}-W\d\d-\d/],["GGGG-[W]WW",/\d{4}-W\d\d/,!1],["YYYY-DDD",/\d{4}-\d{3}/],["YYYY-MM",/\d{4}-\d\d/,!1],["YYYYYYMMDD",/[+-]\d{10}/],["YYYYMMDD",/\d{8}/],["GGGG[W]WWE",/\d{4}W\d{3}/],["GGGG[W]WW",/\d{4}W\d{2}/,!1],["YYYYDDD",/\d{7}/]],isoTimes=[["HH:mm:ss.SSSS",/\d\d:\d\d:\d\d\.\d+/],["HH:mm:ss,SSSS",/\d\d:\d\d:\d\d,\d+/],["HH:mm:ss",/\d\d:\d\d:\d\d/],["HH:mm",/\d\d:\d\d/],
...
addWeekYearFormatToken("GGGGG","isoWeekYear");addUnitAlias("weekYear","gg");addUnitAlias("isoWeekYear","GG");priorities.weekYear=1;priorities.isoWeekYear=1;addRegexToken("G",matchSigned);addRegexToken("g",matchSigned);addRegexToken("GG",match1to2,match2);addRegexToken("gg",match1to2,match2);addRegexToken("GGGG",match1to4,match4);addRegexToken("gggg",match1to4,match4);addRegexToken("GGGGG",match1to6,match6);addRegexToken("ggggg",match1to6,match6);addWeekParseToken(["gggg","ggggg","GGGG","GGGGG"],function(input,
week,config,token){week[token.substr(0,2)]=toInt(input)});addWeekParseToken(["gg","GG"],function(input,week,config,token){week[token]=hooks.parseTwoDigitYear(input)});addFormatToken("Q",0,"Qo","quarter");addUnitAlias("quarter","Q");priorities.quarter=7;addRegexToken("Q",match1);addParseToken("Q",function(input,array){array[MONTH]=3*(toInt(input)-1)});addFormatToken("D",["DD",2],"Do","date");addUnitAlias("date","D");priorities.date=9;addRegexToken("D",match1to2);addRegexToken("DD",match1to2,match2);
addRegexToken("Do",function(isStrict,locale){return isStrict?locale._dayOfMonthOrdinalParse||locale._ordinalParse:locale._dayOfMonthOrdinalParseLenient});addParseToken(["D","DD"],DATE);addParseToken("Do",function(input,array){array[DATE]=toInt(input.match(match1to2)[0])});var getSetDayOfMonth=makeGetSet("Date",!0);addFormatToken("DDD",["DDDD",3],"DDDo","dayOfYear");addUnitAlias("dayOfYear","DDD");priorities.dayOfYear=4;addRegexToken("DDD",match1to3);addRegexToken("DDDD",match3);addParseToken(["DDD",
"DDDD"],function(input,array,config){config._dayOfYear=toInt(input)});addFormatToken("m",["mm",2],0,"minute");addUnitAlias("minute","m");priorities.minute=14;addRegexToken("m",match1to2);addRegexToken("mm",match1to2,match2);addParseToken(["m","mm"],MINUTE);var getSetMinute=makeGetSet("Minutes",!1);addFormatToken("s",["ss",2],0,"second");addUnitAlias("second","s");priorities.second=15;addRegexToken("s",match1to2);addRegexToken("ss",match1to2,match2);addParseToken(["s","ss"],SECOND);var getSetSecond=
makeGetSet("Seconds",!1);addFormatToken("S",0,0,function(){return~~(this.millisecond()/100)});addFormatToken(0,["SS",2],0,function(){return~~(this.millisecond()/10)});addFormatToken(0,["SSS",3],0,"millisecond");addFormatToken(0,["SSSS",4],0,function(){return 10*this.millisecond()});addFormatToken(0,["SSSSS",5],0,function(){return 100*this.millisecond()});addFormatToken(0,["SSSSSS",6],0,function(){return 1E3*this.millisecond()});addFormatToken(0,["SSSSSSS",7],0,function(){return 1E4*this.millisecond()});
addFormatToken(0,["SSSSSSSS",8],0,function(){return 1E5*this.millisecond()});addFormatToken(0,["SSSSSSSSS",9],0,function(){return 1E6*this.millisecond()});addUnitAlias("millisecond","ms");priorities.millisecond=16;addRegexToken("S",match1to3,match1);addRegexToken("SS",match1to3,match2);addRegexToken("SSS",match1to3,match3);var token$jscomp$1;for(token$jscomp$1="SSSS";9>=token$jscomp$1.length;token$jscomp$1+="S")addRegexToken(token$jscomp$1,matchUnsigned);for(token$jscomp$1="S";9>=token$jscomp$1.length;token$jscomp$1+=
"S")addParseToken(token$jscomp$1,parseMs);var getSetMillisecond=makeGetSet("Milliseconds",!1);addFormatToken("z",0,0,"zoneAbbr");addFormatToken("zz",0,0,"zoneName");var proto=Moment.prototype;proto.add=add;proto.calendar=function(time,formats){time=time||createLocal();var sod=cloneWithOffset(time,this).startOf("day");sod=hooks.calendarFormat(this,sod)||"sameElse";formats=formats&&(isFunction(formats[sod])?formats[sod].call(this,time):formats[sod]);return this.format(formats||this.localeData().calendar(sod,
this,createLocal(time)))};proto.clone=function(){return new Moment(this)};proto.diff=function(input,units,asFloat){if(!this.isValid())return NaN;input=cloneWithOffset(input,this);if(!input.isValid())return NaN;var zoneDelta=6E4*(input.utcOffset()-this.utcOffset());units=normalizeUnits(units);switch(units){case "year":units=monthDiff(this,input)/12;break;case "month":units=monthDiff(this,input);break;case "quarter":units=monthDiff(this,input)/3;break;case "second":units=(this-input)/1E3;break;case "minute":units=
(this-input)/6E4;break;case "hour":units=(this-input)/36E5;break;case "day":units=(this-input-zoneDelta)/864E5;break;case "week":units=(this-input-zoneDelta)/6048E5;break;default:units=this-input}return asFloat?units:absFloor(units)};proto.endOf=function(units){units=normalizeUnits(units);if(void 0===units||"millisecond"===units||!this.isValid())return this;var startOfDate=this._isUTC?utcStartOfDate:localStartOfDate;switch(units){case "year":var time=startOfDate(this.year()+1,0,1)-1;break;case "quarter":time=
...
value=getPrioritizedUnits(units);for(var i=0;i<value.length;i++)this[value[i].unit](units[value[i].unit])}else if(units=normalizeUnits(units),isFunction(this[units]))return this[units](value);return this};proto.startOf=function(units){units=normalizeUnits(units);if(void 0===units||"millisecond"===units||!this.isValid())return this;var startOfDate=this._isUTC?utcStartOfDate:localStartOfDate;switch(units){case "year":var time=startOfDate(this.year(),0,1);break;case "quarter":time=startOfDate(this.year(),
this.month()-this.month()%3,1);break;case "month":time=startOfDate(this.year(),this.month(),1);break;case "week":time=startOfDate(this.year(),this.month(),this.date()-this.weekday());break;case "isoWeek":time=startOfDate(this.year(),this.month(),this.date()-(this.isoWeekday()-1));break;case "day":case "date":time=startOfDate(this.year(),this.month(),this.date());break;case "hour":time=this._d.valueOf();time-=mod$1(time+(this._isUTC?0:6E4*this.utcOffset()),36E5);break;case "minute":time=this._d.valueOf();
time-=mod$1(time,6E4);break;case "second":time=this._d.valueOf(),time-=mod$1(time,1E3)}this._d.setTime(time);hooks.updateOffset(this,!0);return this};proto.subtract=subtract;proto.toArray=function(){return[this.year(),this.month(),this.date(),this.hour(),this.minute(),this.second(),this.millisecond()]};proto.toObject=function(){return{years:this.year(),months:this.month(),date:this.date(),hours:this.hours(),minutes:this.minutes(),seconds:this.seconds(),milliseconds:this.milliseconds()}};proto.toDate=
function(){return new Date(this.valueOf())};proto.toISOString=function(keepOffset){if(!this.isValid())return null;var m=(keepOffset=!0!==keepOffset)?this.clone().utc():this;return 0>m.year()||9999<m.year()?formatMoment(m,keepOffset?"YYYYYY-MM-DD[T]HH:mm:ss.SSS[Z]":"YYYYYY-MM-DD[T]HH:mm:ss.SSSZ"):isFunction(Date.prototype.toISOString)?keepOffset?this.toDate().toISOString():(new Date(this.valueOf()+6E4*this.utcOffset())).toISOString().replace("Z",formatMoment(m,"Z")):formatMoment(m,keepOffset?"YYYY-MM-DD[T]HH:mm:ss.SSS[Z]":
"YYYY-MM-DD[T]HH:mm:ss.SSSZ")};proto.inspect=function(){if(!this.isValid())return"moment.invalid(/* "+this._i+" */)";var func="moment",zone="";this.isLocal()||(func=0===this.utcOffset()?"moment.utc":"moment.parseZone",zone="Z");func="["+func+'("]';var year=0<=this.year()&&9999>=this.year()?"YYYY":"YYYYYY";return this.format(func+year+"-MM-DD[T]HH:mm:ss.SSS"+(zone+'[")]'))};proto.toJSON=function(){return this.isValid()?this.toISOString():null};proto.toString=function(){return this.clone().locale("en").format("ddd MMM DD YYYY HH:mm:ss [GMT]ZZ")};
...
this._monthsShortRegex;hasOwnProp(this,"_monthsShortRegex")||(this._monthsShortRegex=matchWord);return this._monthsShortStrictRegex&&isStrict?this._monthsShortStrictRegex:this._monthsShortRegex};proto$1.week=function(mom){return weekOfYear(mom,this._week.dow,this._week.doy).week};proto$1.firstDayOfYear=function(){return this._week.doy};proto$1.firstDayOfWeek=function(){return this._week.dow};proto$1.weekdays=function(m,format){format=isArray(this._weekdays)?this._weekdays:this._weekdays[m&&!0!==m&&
this._weekdays.isFormat.test(format)?"format":"standalone"];return!0===m?shiftWeekdays(format,this._week.dow):m?format[m.day()]:format};proto$1.weekdaysMin=function(m){return!0===m?shiftWeekdays(this._weekdaysMin,this._week.dow):m?this._weekdaysMin[m.day()]:this._weekdaysMin};proto$1.weekdaysShort=function(m){return!0===m?shiftWeekdays(this._weekdaysShort,this._week.dow):m?this._weekdaysShort[m.day()]:this._weekdaysShort};proto$1.weekdaysParse=function(weekdayName,format,strict){var i;if(this._weekdaysParseExact)return handleStrictParse$1.call(this,
weekdayName,format,strict);this._weekdaysParse||(this._weekdaysParse=[],this._minWeekdaysParse=[],this._shortWeekdaysParse=[],this._fullWeekdaysParse=[]);for(i=0;7>i;i++){var mom=createUTC([2E3,1]).day(i);strict&&!this._fullWeekdaysParse[i]&&(this._fullWeekdaysParse[i]=new RegExp("^"+this.weekdays(mom,"").replace(".","\\.?")+"$","i"),this._shortWeekdaysParse[i]=new RegExp("^"+this.weekdaysShort(mom,"").replace(".","\\.?")+"$","i"),this._minWeekdaysParse[i]=new RegExp("^"+this.weekdaysMin(mom,"").replace(".",
"\\.?")+"$","i"));this._weekdaysParse[i]||(mom="^"+this.weekdays(mom,"")+"|^"+this.weekdaysShort(mom,"")+"|^"+this.weekdaysMin(mom,""),this._weekdaysParse[i]=new RegExp(mom.replace(".",""),"i"));if(strict&&"dddd"===format&&this._fullWeekdaysParse[i].test(weekdayName)||strict&&"ddd"===format&&this._shortWeekdaysParse[i].test(weekdayName)||strict&&"dd"===format&&this._minWeekdaysParse[i].test(weekdayName)||!strict&&this._weekdaysParse[i].test(weekdayName))return i}};proto$1.weekdaysRegex=function(isStrict){if(this._weekdaysParseExact)return hasOwnProp(this,
"_weekdaysRegex")||computeWeekdaysParse.call(this),isStrict?this._weekdaysStrictRegex:this._weekdaysRegex;hasOwnProp(this,"_weekdaysRegex")||(this._weekdaysRegex=matchWord);return this._weekdaysStrictRegex&&isStrict?this._weekdaysStrictRegex:this._weekdaysRegex};proto$1.weekdaysShortRegex=function(isStrict){if(this._weekdaysParseExact)return hasOwnProp(this,"_weekdaysRegex")||computeWeekdaysParse.call(this),isStrict?this._weekdaysShortStrictRegex:this._weekdaysShortRegex;hasOwnProp(this,"_weekdaysShortRegex")||
(this._weekdaysShortRegex=matchWord);return this._weekdaysShortStrictRegex&&isStrict?this._weekdaysShortStrictRegex:this._weekdaysShortRegex};proto$1.weekdaysMinRegex=function(isStrict){if(this._weekdaysParseExact)return hasOwnProp(this,"_weekdaysRegex")||computeWeekdaysParse.call(this),isStrict?this._weekdaysMinStrictRegex:this._weekdaysMinRegex;hasOwnProp(this,"_weekdaysMinRegex")||(this._weekdaysMinRegex=matchWord);return this._weekdaysMinStrictRegex&&isStrict?this._weekdaysMinStrictRegex:this._weekdaysMinRegex};
proto$1.isPM=function(input){return"p"===(input+"").toLowerCase().charAt(0)};proto$1.meridiem=function(hours,minutes,isLower){return 11<hours?isLower?"pm":"PM":isLower?"am":"AM"};getSetGlobalLocale("en",{dayOfMonthOrdinalParse:/\d{1,2}(th|st|nd|rd)/,ordinal:function(number){var b=number%10;b=1===toInt(number%100/10)?"th":1===b?"st":2===b?"nd":3===b?"rd":"th";return number+b}});hooks.lang=deprecate("moment.lang is deprecated. Use moment.locale instead.",getSetGlobalLocale);hooks.langData=deprecate("moment.langData is deprecated. Use moment.localeData instead.",
getLocale);var mathAbs=Math.abs,asMilliseconds=makeAs("ms"),asSeconds=makeAs("s"),asMinutes=makeAs("m"),asHours=makeAs("h"),asDays=makeAs("d"),asWeeks=makeAs("w"),asMonths=makeAs("M"),asQuarters=makeAs("Q"),asYears=makeAs("y"),milliseconds=makeGetter("milliseconds"),seconds=makeGetter("seconds"),minutes=makeGetter("minutes"),hours=makeGetter("hours"),days=makeGetter("days"),months=makeGetter("months"),years=makeGetter("years"),round=Math.round,thresholds={ss:44,s:45,m:45,h:22,d:26,M:11},abs$1=Math.abs,
proto$2=Duration.prototype;proto$2.isValid=function(){return this._isValid};proto$2.abs=function(){var data=this._data;this._milliseconds=mathAbs(this._milliseconds);this._days=mathAbs(this._days);this._months=mathAbs(this._months);data.milliseconds=mathAbs(data.milliseconds);data.seconds=mathAbs(data.seconds);data.minutes=mathAbs(data.minutes);data.hours=mathAbs(data.hours);data.months=mathAbs(data.months);data.years=mathAbs(data.years);return this};proto$2.add=function(input,value){return addSubtract$1(this,
input,value,1)};proto$2.subtract=function(input,value){return addSubtract$1(this,input,value,-1)};proto$2.as=function(units){if(!this.isValid())return NaN;var milliseconds=this._milliseconds;units=normalizeUnits(units);if("month"===units||"quarter"===units||"year"===units){var days=this._days+milliseconds/864E5;days=this._months+4800*days/146097;switch(units){case "month":return days;case "quarter":return days/3;case "year":return days/12}}else switch(days=this._days+Math.round(146097*this._months/
4800),units){case "week":return days/7+milliseconds/6048E5;case "day":return days+milliseconds/864E5;case "hour":return 24*days+milliseconds/36E5;case "minute":return 1440*days+milliseconds/6E4;case "second":return 86400*days+milliseconds/1E3;case "millisecond":return Math.floor(864E5*days)+milliseconds;default:throw Error("Unknown unit "+units);}};proto$2.asMilliseconds=asMilliseconds;proto$2.asSeconds=asSeconds;proto$2.asMinutes=asMinutes;proto$2.asHours=asHours;proto$2.asDays=asDays;proto$2.asWeeks=
asWeeks;proto$2.asMonths=asMonths;proto$2.asQuarters=asQuarters;proto$2.asYears=asYears;proto$2.valueOf=function(){return this.isValid()?this._milliseconds+864E5*this._days+this._months%12*2592E6+31536E6*toInt(this._months/12):NaN};proto$2._bubble=function(){var milliseconds=this._milliseconds,days=this._days,months=this._months,data=this._data;0<=milliseconds&&0<=days&&0<=months||0>=milliseconds&&0>=days&&0>=months||(milliseconds+=864E5*absCeil(146097*months/4800+days),months=days=0);data.milliseconds=
milliseconds%1E3;milliseconds=absFloor(milliseconds/1E3);data.seconds=milliseconds%60;milliseconds=absFloor(milliseconds/60);data.minutes=milliseconds%60;milliseconds=absFloor(milliseconds/60);data.hours=milliseconds%24;days+=absFloor(milliseconds/24);milliseconds=absFloor(4800*days/146097);months+=milliseconds;days-=absCeil(146097*milliseconds/4800);milliseconds=absFloor(months/12);data.days=days;data.months=months%12;data.years=milliseconds;return this};proto$2.clone=function(){return createDuration(this)};
proto$2.get=function(units){units=normalizeUnits(units);return this.isValid()?this[units+"s"]():NaN};proto$2.milliseconds=milliseconds;proto$2.seconds=seconds;proto$2.minutes=minutes;proto$2.hours=hours;proto$2.days=days;proto$2.weeks=function(){return absFloor(this.days()/7)};proto$2.months=months;proto$2.years=years;proto$2.humanize=function(withSuffix){if(!this.isValid())return this.localeData().invalidDate();var locale=this.localeData();var output=!withSuffix;var duration=createDuration(this).abs(),
seconds=round(duration.as("s")),minutes=round(duration.as("m")),hours=round(duration.as("h")),days=round(duration.as("d")),months=round(duration.as("M"));duration=round(duration.as("y"));seconds=seconds<=thresholds.ss&&["s",seconds]||seconds<thresholds.s&&["ss",seconds]||1>=minutes&&["m"]||minutes<thresholds.m&&["mm",minutes]||1>=hours&&["h"]||hours<thresholds.h&&["hh",hours]||1>=days&&["d"]||days<thresholds.d&&["dd",days]||1>=months&&["M"]||months<thresholds.M&&["MM",months]||1>=duration&&["y"]||
["yy",duration];seconds[2]=output;seconds[3]=0<+this;seconds[4]=locale;output=substituteTimeAgo.apply(null,seconds);withSuffix&&(output=locale.pastFuture(+this,output));return locale.postformat(output)};proto$2.toISOString=toISOString$1;proto$2.toString=toISOString$1;proto$2.toJSON=toISOString$1;proto$2.locale=locale$jscomp$1;proto$2.localeData=localeData;proto$2.toIsoString=deprecate("toIsoString() is deprecated. Please use toISOString() instead (notice the capitals)",toISOString$1);proto$2.lang=
lang;addFormatToken("X",0,0,"unix");addFormatToken("x",0,0,"valueOf");addRegexToken("x",matchSigned);addRegexToken("X",/[+-]?\d+(\.\d{1,3})?/);addParseToken("X",function(input,array,config){config._d=new Date(1E3*parseFloat(input,10))});addParseToken("x",function(input,array,config){config._d=new Date(toInt(input))});hooks.version="2.24.0";var hookCallback=createLocal;hooks.fn=proto;hooks.min=function(){var args=[].slice.call(arguments,0);return pickBy("isBefore",args)};hooks.max=function(){var args=
[].slice.call(arguments,0);return pickBy("isAfter",args)};hooks.now=function(){return Date.now?Date.now():+new Date};hooks.utc=createUTC;hooks.unix=function(input){return createLocal(1E3*input)};hooks.months=function(format,index){return listMonthsImpl(format,index,"months")};hooks.isDate=isDate;hooks.locale=getSetGlobalLocale;hooks.invalid=createInvalid;hooks.duration=createDuration;hooks.isMoment=isMoment;hooks.weekdays=function(localeSorted,format,index){return listWeekdaysImpl(localeSorted,format,
index,"weekdays")};hooks.parseZone=function(){return createLocal.apply(null,arguments).parseZone()};hooks.localeData=getLocale;hooks.isDuration=isDuration;hooks.monthsShort=function(format,index){return listMonthsImpl(format,index,"monthsShort")};hooks.weekdaysMin=function(localeSorted,format,index){return listWeekdaysImpl(localeSorted,format,index,"weekdaysMin")};hooks.defineLocale=defineLocale;hooks.updateLocale=function(name,config){if(null!=config){var parentConfig=baseConfig;var tmpLocale=loadLocale(name);
null!=tmpLocale&&(parentConfig=tmpLocale._config);config=mergeConfigs(parentConfig,config);config=new Locale(config);config.parentLocale=locales[name];locales[name]=config;getSetGlobalLocale(name)}else null!=locales[name]&&(null!=locales[name].parentLocale?locales[name]=locales[name].parentLocale:null!=locales[name]&&delete locales[name]);return locales[name]};hooks.locales=function(){return keys(locales)};hooks.weekdaysShort=function(localeSorted,format,index){return listWeekdaysImpl(localeSorted,
format,index,"weekdaysShort")};hooks.normalizeUnits=normalizeUnits;hooks.relativeTimeRounding=function(roundingFunction){return void 0===roundingFunction?round:"function"===typeof roundingFunction?(round=roundingFunction,!0):!1};hooks.relativeTimeThreshold=function(threshold,limit){if(void 0===thresholds[threshold])return!1;if(void 0===limit)return thresholds[threshold];thresholds[threshold]=limit;"s"===threshold&&(thresholds.ss=limit-1);return!0};hooks.calendarFormat=function(myMoment,now){myMoment=
myMoment.diff(now,"days",!0);return-6>myMoment?"sameElse":-1>myMoment?"lastWeek":0>myMoment?"lastDay":1>myMoment?"sameDay":2>myMoment?"nextDay":7>myMoment?"nextWeek":"sameElse"};hooks.prototype=proto;hooks.HTML5_FMT={DATETIME_LOCAL:"YYYY-MM-DDTHH:mm",DATETIME_LOCAL_SECONDS:"YYYY-MM-DDTHH:mm:ss",DATETIME_LOCAL_MS:"YYYY-MM-DDTHH:mm:ss.SSS",DATE:"YYYY-MM-DD",TIME:"HH:mm",TIME_SECONDS:"HH:mm:ss",TIME_MS:"HH:mm:ss.SSS",WEEK:"GGGG-[W]WW",MONTH:"YYYY-MM"};return hooks})}
//# sourceMappingURL=module$node_modules$moment$moment.js.map
